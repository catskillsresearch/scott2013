...
Pipeline:
  ...
  3. Demote the Appendix-A structural headings so LaTeX numbers everything once, then
     insert \\appendix before the combined Agda-source appendix.
...
from listing_sanitize import chunk_line_ranges, sanitize_for_arxiv

SRC = ROOT / "arxiv_with_code.md"
OUT = ROOT / "arxiv.tex"
PREAMBLE = SCRIPTS / "tex_preamble_arxiv.tex"
LISTINGS_DIR = ROOT / "agda-listings"
...
AGDA_MODULE_RE = re.compile(r"^###\s+(Scott2013(?:\.agda|/[^\s{]+))\s*$", re.MULTILINE)
...
def normalize_appendix_headings(text: str) -> str:
    text = re.sub(
        r"^#\s+Appendix A: (?:Complete Agda source|Agda module index)\s*$",
        "## Agda module index",
        text,
        flags=re.MULTILINE,
    )
    text = re.sub(
        r"^##\s+`(Scott2013(?:\.agda|/[^`]+))`\s*$",
        r"### \1",
        text,
        flags=re.MULTILINE,
    )
...
def write_listing(code: str, listing_name: str) -> tuple[str, int]:
    ...
    source = sanitize_for_arxiv(code.rstrip("\n"))
...
def agda_block_latex(code: str, listing_name: str) -> str:
    ...
    for first, last in ranges:
        if first == 1 and last == line_count:
            parts.append(
                ...
                "\\noindent\\textcolor{green!40!black}{\\textbf{Agda source}}"
                "\\par\\vspace{0.25\\baselineskip}\n"
                f"\\lstinputlisting[style=agdabox]{{{rel_path}}}\n"
                "\\vspace{0.5\\baselineskip}\n\n"
            )
        else:
            parts.append(
                ...
                f"\\lstinputlisting[style=agdabox,firstline={first},lastline={last}]"
                f"{{{rel_path}}}\n\n"
            )
    return "".join(parts)
...
def extract_agda_titles(text: str) -> dict[str, str]:
    titles: dict[str, str] = {}
    agda_starts = [m.start() for m in re.finditer(r"^```agda\s*$", text, re.MULTILINE)]
    for idx, pos in enumerate(agda_starts):
        prefix = text[:pos].rstrip("\n")
        module = None
        for line in reversed(prefix.splitlines()[-4:]):
            m = re.match(r"^###\s+(Scott2013(?:\.agda|/[^\s{]+))", line.strip())
            if m:
                module = m.group(1)
                break
        titles[f"AGDAINCLUDE{idx:03d}"] = module or f"module-{idx + 1}"
    return titles


def replace_fences(text: str, figure_captions: list[str]) -> tuple[str, dict[str, str]]:
    agda_titles = extract_agda_titles(text)
    ...
    def repl(match: re.Match[str]) -> str:
        nonlocal agda_idx, other_idx, figure_idx
        lang = match.group(1).strip().lower()
        body = match.group(2)
        if lang == "agda":
            key = f"AGDAINCLUDE{agda_idx:03d}"
            module = agda_titles.get(key, f"module-{agda_idx}")
            agda_idx += 1
            safe_name = module.replace("/", "-")
            if not safe_name.endswith(".agda"):
                safe_name += ".agda"
            placeholders[key] = agda_block_latex(body, safe_name)
            return f"\n\n{key}\n\n"
        if lang == "math":
        ...
        if lang == "mermaid":
            ...
            label = f"fig:scott2013-{figure_idx + 1:02d}"
            figure_idx += 1
            other_idx += 1
        ...
        other_idx += 1
        placeholders[key] = f"\\lstinputlisting[style=agdabox]{{{rel_path}}}\n"
        return f"\n\n{key}\n\n"
...
def break_texttt_paths(latex: str) -> str:
    ...
    def fix(match: re.Match[str]) -> str:
        ...
        inner = inner.replace(".agda", ".\\allowbreak{}agda")
...
def cleanup_pandoc_latex(latex: str) -> str:
    ...
    latex = re.sub(
        r"\\section\{Appendix A\. Agda source index\}",
        "",
        latex,
    )
    latex = re.sub(
        r"\\section\{Appendix A: (?:Complete Agda source|Agda module index)\}",
        r"\\section{Agda module index}",
        latex,
    )
    latex = re.sub(
        r"\\section\{Appendix A\. Agda module index\}",
        r"\\section{Agda module index}",
        latex,
    )
    latex = re.sub(r"\n{3,}", "\n\n", latex)
...
def insert_appendix_command(latex: str) -> str:
    marker = r"\section{Agda module index}"
